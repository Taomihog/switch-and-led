...
template <typename T>
void keep_unique_rows(std::vector<T>& matrix) {
    std::sort(matrix.begin(), matrix.end());
    matrix.erase(std::unique(matrix.begin(), matrix.end()), matrix.end());
}

template <typename T>
[[nodiscard]] inline size_t n_unique_sorted(const std::vector<T>& vec) {
...
template <typename T>
[[nodiscard]] int stdev(const std::vector<T>& arr) {
...
template <typename T>
[[nodiscard]] int n1s(const std::vector<T>& arr) { // number of bit1s
    int n1s = 0;
    for (const auto& r : arr) {
        n1s += std::popcount(r);
    }
...
template<typename T>
[[nodiscard]] static std::vector<T> pack_matrix(const std::vector<T>& matrix, T allowed_mask) {
...
template<typename T>
[[nodiscard]] T unpack_mask(T packed_mask, T allowed_mask) {
    T result = 0;
    T temp_allowed = allowed_mask;
    int source_bit = 0;
    while (temp_allowed != 0) {
