...
template<typename T> 
[[nodiscard]] std::pair<T, std::vector<std::vector<size_t>>> relevant_bits(const tt_t<T>& tt) {
...
template<typename T>
[[nodiscard]] int reachable_distance(T r1, T r2, int V) {
...
template<typename T>
[[nodiscard]] uint32_t sum_output_distance_for_adjacent_inputs(int dist, const tt_t<T>& tt) {
